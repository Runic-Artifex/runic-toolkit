...
[CS-WebUI](https://github.com/Runic-Artifex/cs-webui) binding for WebUI. The first
...
useful composition libraries, but neither requires ASP.NET Core. CS-WebUI already
...
- Desktop samples use local HTML, CSS, and JavaScript served by CS-WebUI. Early
...
- The Nix development shell supplies the native WebUI library and Linux browser
  and webview runtime dependencies required by CS-WebUI.

## Consequences

The normal application path is a native CS-WebUI process rather than a localhost
