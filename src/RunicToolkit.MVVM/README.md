# RunicToolkit.MVVM v1 runtime

`RunicToolkit.MVVM` is the BCL-only, `net10.0` contract and session kernel for
`runic.toolkit.mvvm/1`. It deliberately does not reference Hosting, Flow, a frontend
...
The canonical schema, wire corpus, and semantic corpus are owned here. Each
integration owns executable evidence in its own repository and consumes the
public protocol/package boundary without adding integration-specific vocabulary
to MVVM core. Binding-adapter owners use the complete-vocabulary builder,
explicit delegates, `OnDispose` for owned subscriptions, and a completed
`MvvmBindingResult` that represents one atomic commit.

## Diagnostics

The runtime emits BCL `ActivitySource` and `Meter` instrumentation under the stable
name exposed by `MvvmDiagnostics.InstrumentationName` (`RunicToolkit.MVVM`). This
