...
public sealed class ApplicationBridgeGenerator : IIncrementalGenerator
{
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<AdditionalText> files)
    {
        ...
        try
        {
            ...
            ContractModel model = ParseManifest(document.RootElement);
    ...
    private static ContractModel ParseManifest(JsonElement root)
    {
        ...
        var commands = root.GetProperty("commands").EnumerateArray()
            ...
            .ToImmutableArray();
        string contractFingerprint = root.GetProperty("contractFingerprint").GetString()!;
        ...
        string actualFingerprint = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(
            CanonicalizeManifestBody(root)))).ToLowerInvariant();
    ...
    private static string CanonicalizeManifestBody(JsonElement root)
    {
        var source = new StringBuilder();
        WriteCanonicalJson(source, root, 0, skipContractFingerprint: true);
        source.Append('\n');
        return source.ToString();
    }

    private static void WriteCanonicalJson(StringBuilder source, JsonElement element, int depth, bool skipContractFingerprint = false)
    {
        switch (element.ValueKind)
        {
            case JsonValueKind.Object:
            {
                JsonProperty[] properties = element.EnumerateObject()
                    .Where(property => !skipContractFingerprint || property.Name != "contractFingerprint")
