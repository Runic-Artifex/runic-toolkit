...
export async function generateFrontendContracts(options) {
  ...
  if (source.$schema !== "runic.toolkit.mvvm.frontend-contract/1") {
  ...
  for (const [path, renderer] of [
    ...
    [options.vuePath, vue],
    [options.sveltePath, svelte],
    [options.angularPath, angular],
  ]) {
    if (path !== undefined) {
      const outputPath = resolve(path);
      await emit(
        outputPath,
        renderer(source.contracts, moduleImport(outputPath, typescriptPath)),
        options.verify === true,
      );
    }
  }
...
function csharp(source) {
  ...
  for (const contract of source.contracts) {
    ...
    lines.push("        }", "");
    lines.push(
      `        internal static global::RunicToolkit.MVVM.CommunityToolkit.CommunityToolkitMvvmBindingAdapter<${contract.csharp.modelType}> CreateAdapter(`,
      `            ${contract.csharp.modelType} model) =>`,
      `            new global::RunicToolkit.MVVM.CommunityToolkit.CommunityToolkitMvvmAdapterBuilder<${contract.csharp.modelType}>(model)`);
    for (const member of contract.members) {
...
function typescript(contracts) {
  ...
  for (const contract of contracts) {
    ...
    lines.push("  public static readonly memberMetadata = [");
    for (const member of contract.members) {
      const modelType = contract.csharp.modelType.replace(/^global::/, "");
      lines.push(
        "    { " +
        `id: ${member.id}, ` +
...
function react(contracts, contractImport) {
  const lines = [
    "// <auto-generated />",
    "import {",
    "  useMvvmCollection,",
    ...
    `import type { ${contracts.map((contract) => `${contract.client}Contract`).join(", ")} } from ${JSON.stringify(contractImport)};`,
    "",
  ];
  for (const contract of contracts) {
    const functionName = `use${contract.client}Bindings`;
    lines.push(
      `export function ${functionName}(contract: ${contract.client}Contract) {`);
    for (const member of contract.members) {
      lines.push(`  const ${bindingName(member)} = ${reactBinding(member)};`);
      if (member.validation === true) {
        lines.push(
          `  const ${member.name}Errors = useMvvmValidation(contract.${member.name});`);
      }
    }
    lines.push("  return {", "    contract,");
    for (const member of contract.members) {
      lines.push(`    ${member.name}: ${bindingName(member)},`);
      if (member.validation === true) lines.push(`    ${member.name}Errors,`);
    }
    lines.push("  } as const;", "}", "");
  }
  return lines.join("\n");
...
function reactBinding(member) {
  if (member.kind === "property") {
    return `useMvvmProperty(contract.${member.name})`;
  }
  if (member.kind === "collection") {
    return `useMvvmCollection(contract.${member.name})`;
  }
  return `useMvvmCommandFacade(contract.${member.name})`;
}

function vue(contracts, contractImport) {
  const lines = [
    "// <auto-generated />",
    "import {",
  ...
  for (const contract of contracts) {
    lines.push(
      `export function use${contract.client}Bindings(`,
      `  contract: ${contract.client}Contract,`,
      "  adapter?: VueMvvmAdapter,",
      ") {");
    for (const member of contract.members) {
      lines.push(`  const ${bindingName(member)} = ${vueBinding(member)};`);
      if (member.validation === true) {
        lines.push(
          `  const ${member.name}Errors = useVueMvvmValidation(contract.${member.name}, adapter);`);
      }
    }
    lines.push("  return {", "    contract,");
    for (const member of contract.members) {
...
function vueBinding(member) {
  if (member.kind === "property") {
    return `useVueMvvmProperty(contract.${member.name}, adapter)`;
  }
  if (member.kind === "collection") {
    return `useVueMvvmCollection(contract.${member.name}, adapter)`;
  }
  return `useVueMvvmCommandFacade(contract.${member.name}, adapter)`;
}

function svelte(contracts, contractImport) {
  const lines = [
    "// <auto-generated />",
    "import {",
  ...
  for (const contract of contracts) {
    lines.push(
      `export function create${contract.client}Stores(`,
      "  store: SvelteMvvmStore,",
      `  contract: ${contract.client}Contract,`,
      ") {");
    for (const member of contract.members) {
      lines.push(`  const ${bindingName(member)} = ${svelteBinding(member)};`);
      if (member.validation === true) {
        lines.push(
          `  const ${member.name}Errors = derivedMvvmValidation(store, contract.${member.name});`);
      }
    }
    lines.push("  return {", "    contract,");
    for (const member of contract.members) {
    ...
    const commandBindings = contract.members.filter(
      (member) => member.kind === "command");
    if (commandBindings.length !== 0) {
      lines.push("    dispose() {");
      for (const member of commandBindings) {
        lines.push(`      ${bindingName(member)}.dispose();`);
      }
      lines.push("    },");
    }
    lines.push("  } as const;", "}", "");
...
function svelteBinding(member) {
  if (member.kind === "property") {
    return `derivedMvvmProperty(store, contract.${member.name})`;
  }
  if (member.kind === "collection") {
    return `derivedMvvmCollection(store, contract.${member.name})`;
  }
  return `createSvelteMvvmCommandFacade(store, contract.${member.name})`;
}

function angular(contracts, contractImport) {
  const lines = [
    "// <auto-generated />",
    "import { inject, type Provider } from \"@angular/core\";",
    "import { AngularMvvmStore } from \"@runic-artifex/mvvm-angular\";",
    `import type { ${contracts.map((contract) => `${contract.client}Contract`).join(", ")} } from ${JSON.stringify(contractImport)};`,
    "",
  ];
  for (const contract of contracts) {
    const service = `${contract.client}ContractService`;
    lines.push(
      `export class ${service} {`);
    for (const member of contract.members) {
      lines.push(`  public readonly ${member.name};`);
      if (member.validation === true) {
        lines.push(`  public readonly ${member.name}Errors;`);
      }
    }
    lines.push(
      "",
      ...
      `    public readonly contract: ${contract.client}Contract,`,
      "  ) {");
    for (const member of contract.members) {
      lines.push(`    this.${member.name} = ${angularBinding(member)};`);
      if (member.validation === true) {
        lines.push(
          `    this.${member.name}Errors = store.validation(contract.${member.name});`);
      }
    }
    lines.push(
      "  }",
      "}",
      "",
      `export function provide${contract.client}Contract(`,
      "  store: AngularMvvmStore,",
      `  contract: ${contract.client}Contract,`,
      "): readonly Provider[] {",
      `  return [{ provide: ${service}, useFactory: () => new ${service}(store, contract) }];`,
      "}",
      "",
      `export function inject${contract.client}Contract(): ${service} {`,
...
function angularBinding(member) {
  if (member.kind === "property") {
    return `store.property(contract.${member.name})`;
  }
  if (member.kind === "collection") {
    return `store.collection(contract.${member.name})`;
  }
  return `store.commandFacade(contract.${member.name})`;
}

function moduleImport(fromPath, targetPath) {
  const path = relative(dirname(fromPath), resolve(targetPath))
    .replaceAll("\\", "/")
...
if (process.argv[1] !== undefined &&
  ...
  await generateFrontendContracts({
    ...
    reactPath: values.get("react"),
    vuePath: values.get("vue"),
    sveltePath: values.get("svelte"),
    angularPath: values.get("angular"),
    verify,
  });
}
