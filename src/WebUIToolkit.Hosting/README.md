...
`WebUIToolkit.Hosting` is the framework-neutral, BCL-only composition and lifecycle
kernel for a single-use WebUIToolkit application. It classifies one launch, validates
...
`WebUIToolkitApplicationBuilder` explicitly registers the neutral host, common and
...
returns a single-use `WebUIToolkitApplication`. There is no assembly scanning,
...
decision-shape errors use `WUTHOST1201` and never contain a consumer type name or launch
argument; the lifecycle aggregates any reported validation errors into its terminal
`WUTHOST1001` configuration failure.
...
failure events contain category, exact `WUTHOST` plus four-digit code when valid,
expected status, and
...
`WebUIToolkit.Hosting.Build` constructs deterministic manifests against those asset
