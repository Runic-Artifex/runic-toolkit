...
`RunicToolkit.Collections`. The shipping target is `net10.0`. The implementation is
...
namespace, or the separately reserved `RunicToolkit.Collections.Observable` package
identity.
...
`tests/RunicToolkit.Collections.PackageConsumer` is an isolated generated consumer
...
The Application Bridge and dispatcher are separate integration edges. Framework-
specific collection adapters follow their owning UI integrations.
They must consume this frozen contract without adding an Application Bridge dependency to the
Collections package.
