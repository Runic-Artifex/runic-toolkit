...
namespace WebUIToolkit.Frontend.Generators;

[Generator(LanguageNames.CSharp)]
public sealed class FrontendContractGenerator : IIncrementalGenerator
{
    private const string ContractAttribute =
        "WebUIToolkit.MVVM.WebUiFrontendContractAttribute";
    private const string PropertyAttribute =
        "WebUIToolkit.MVVM.WebUiFrontendPropertyAttribute";
    private const string CollectionAttribute =
        "WebUIToolkit.MVVM.WebUiFrontendCollectionAttribute";
    private const string CommandAttribute =
        "WebUIToolkit.MVVM.WebUiFrontendCommandAttribute";
        ...
        GenerationOptions options)
    {
        ...
        context.AddSource(
            "WebUIToolkit.FrontendContracts.g.cs",
        ...
        out string className)
    {
        ...
        if (options.ArtifactPath.Length == 0)
        {
            Report(context, contracts[0].Location,
                "Set WebUIToolkitFrontendContractArtifact when C#-first contracts are enabled.");
            valid = false;
        }
        ...
        string className)
    {
        ...
        foreach (ContractSpec contract in contracts)
        {
            ...
            output.AppendLine("        }")
                .AppendLine()
                .Append("        internal static global::WebUIToolkit.MVVM.CommunityToolkit.CommunityToolkitMvvmBindingAdapter<")
                .Append(modelType).AppendLine("> CreateAdapter(")
                .Append("            ").Append(modelType).AppendLine(" model) =>")
                .Append("            new global::WebUIToolkit.MVVM.CommunityToolkit.CommunityToolkitMvvmAdapterBuilder<")
        ...
        string ProjectDirectory)
    {
        internal static GenerationOptions Create(AnalyzerConfigOptions options)
        {
            bool enabled = options.TryGetValue(
                "build_property.WebUIToolkitFrontendContractCSharpFirst",
                out string? value)
                && string.Equals(value, "true", StringComparison.OrdinalIgnoreCase);
            options.TryGetValue(
                "build_property.WebUIToolkitFrontendContractArtifact",
                out string? artifact);
            options.TryGetValue(
