...
Decision: promote `WebUIToolkit.Collections` to a G2 kernel/integration candidate.
...
|---|---|---|
| Frozen range and reconciliation protocol | `tests/WebUIToolkit.Collections.Tests` | All OR0–OR2 traces and Wave B model, boundary, property, reentrancy, exception, and adversarial-plan cases pass. |
...
dotnet run --project benchmarks/WebUIToolkit.Collections.Benchmarks -c Release --no-build -- --gate
dotnet run --project benchmarks/WebUIToolkit.Collections.Benchmarks -c Release --no-build -- --full
dotnet run --project tests/WebUIToolkit.Collections.PackageConsumer -c Release --no-build
```
