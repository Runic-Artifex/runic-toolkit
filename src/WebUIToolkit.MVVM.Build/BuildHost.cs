...
namespace WebUIToolkit.MVVM.Build;
...
internal static class BuildHost
{
    ...
    public static int Main(string[] args)
    {
                                         ...
                                         ArgumentException or EncoderFallbackException or DecoderFallbackException)
        {
            Console.Error.WriteLine(string.Create(
                ...
                $"error WUTMVVM0903: {SanitizeMessage(exception.Message)}"));
            return 2;
        }
    }
    ...
    private static int Compile(BuildHostOptions options)
    {
        ...
        foreach (BindingInput input in inputs)
        {
            ...
            foreach (BindingDiagnostic diagnostic in result.Diagnostics)
            {
                ...
                {
                    Console.Error.WriteLine("error WUTMVVM0003: The build diagnostic limit was exceeded; further diagnostics were suppressed.");
                    return 1;
                }
    ...
    private static void ReconcileStaleArtifacts(string outputDirectory, IReadOnlyCollection<string> currentNames)
    {
        ...
        DeleteUnlistedGeneratedFiles(outputDirectory, current, "WebUIToolkit.MVVM.*.g.cs");
        DeleteUnlistedGeneratedFiles(outputDirectory, current, "webuitoolkit.mvvm.*.contract.json");
    }
    ...
    private static int Clean(BuildHostCleanOptions options)
    {
        ...
        DeleteUnlistedGeneratedFiles(outputDirectory, noCurrentNames, "WebUIToolkit.MVVM.*.g.cs");
        DeleteUnlistedGeneratedFiles(outputDirectory, noCurrentNames, "webuitoolkit.mvvm.*.contract.json");
    ...
    {
        const string sourcePrefix = "WebUIToolkit.MVVM.";
        const string sourceSuffix = ".g.cs";
        const string manifestPrefix = "webuitoolkit.mvvm.";
    ...
    private static void WriteUsage() => Console.Error.WriteLine(
        "Usage: WebUIToolkit.MVVM.Build --project-directory <path> --intermediate-directory <path> --output-directory <path> --input-list <path>");
