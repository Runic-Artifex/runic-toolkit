...
The Hosting family owns `RTKHOST0001`-`RTKHOST9999`. Wave A/B allocates the following
...
event retains only an exact `RTKHOST` plus four-ASCII-digit code; foreign or malformed
codes become `null`.
...
`RunicToolkit.Desktop` contract package. Its remaining shipping-project lock entries
