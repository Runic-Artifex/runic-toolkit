...
public static class ApplicationBridgeCodec
{
    ...
    {
        if (string.IsNullOrWhiteSpace(envelope.Protocol) || envelope.Version < 1 ||
            !IsFingerprint(envelope.ContractFingerprint) || envelope.ConnectionEpoch < 0 ||
            envelope.CommandId == Guid.Empty)
        {
            return false;
        }
