...
This package carries the existing `runic.toolkit.mvvm/1` protocol over CsWebUi.
...
`contentFiles/any/any/wwwroot/runic-toolkit-mvvm-cswebui.mjs`. Copy or bundle
that file with the application and compose it with `@runic-artifex/mvvm`:

```js
import { MvvmClient, ProtocolTransport } from "@runic-artifex/mvvm";
