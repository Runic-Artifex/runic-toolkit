...
using RunicToolkit.MVVM.Build.Symbols;

namespace RunicToolkit.MVVM.Build.Tests;

internal static class GeneratedMemberContractTests
{
    private const string FixtureTypeName = "RunicToolkit.MVVM.Build.Tests.Fixtures.GeneratedMemberViewModel";
    ...
    {
        string root = Path.Combine(Path.GetTempPath(), "RunicToolkit.MVVM.GeneratedMember", Guid.NewGuid().ToString("N"));
    ...
    {
        string path = Path.Combine(AppContext.BaseDirectory, "RunicToolkit.MVVM.Build.Tests.dll");
    ...
    {
        string root = Path.Combine(Path.GetTempPath(), "RunicToolkit.MVVM.GeneratedMemberConsumer", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(root, "Program.cs"), $$"""
                using RunicToolkit.MVVM.Build.Tests.Fixtures;
                using RunicToolkit.MVVM.Generated;
            ...
            File.WriteAllText(Path.Combine(root, "Consumer.csproj"), string.Create(CultureInfo.InvariantCulture, $"""
                <Project Sdk="Microsoft.NET.Sdk">
                  ...
                  <ItemGroup>
                    <Reference Include="RunicToolkit.MVVM.Build.Tests">
    ...
    private static string BuildProducer(string root, CultureInfo culture, bool reverseReferences)
    {
        ...
        string packageRoot = Environment.GetEnvironmentVariable("NUGET_PACKAGES") is { Length: > 0 } configuredPackageRoot
            ? Path.GetFullPath(configuredPackageRoot)
            : Path.Combine(FindRepositoryRoot(), ".packages", "nuget");
        ...
        [
            ("RunicToolkit.MVVM.Build", buildAssembly),
            ("RunicToolkit.MVVM.Build.Tests", testAssembly),
        ];
        ...
        File.WriteAllText(Path.Combine(root, "Producer.csproj"), string.Create(CultureInfo.InvariantCulture, $"""
            <Project Sdk="Microsoft.NET.Sdk">
              <PropertyGroup>
                ...
                <RootNamespace>RunicToolkit.MVVM.Build.Tests.Fixtures</RootNamespace>
        ...
        File.WriteAllText(Path.Combine(root, "GeneratedMemberViewModel.cs"), """
            ...
            namespace RunicToolkit.MVVM.Build.Tests.Fixtures;
