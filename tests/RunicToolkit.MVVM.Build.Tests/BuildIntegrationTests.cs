...
internal static class BuildIntegrationTests
{
    ...
    {
        XDocument project = Load("RunicToolkit.MVVM.Build.csproj");
    ...
    {
        XDocument props = Load("buildTransitive", "RunicToolkit.MVVM.Build.props");
        Assert.Equal(".rtkmvvm", Property(props, "RunicToolkitMvvmBindingExtension"));

        XDocument targets = Load("buildTransitive", "RunicToolkit.MVVM.Build.targets");
        XElement item = targets.Descendants("RunicToolkitMvvmBinding").Single();
        Assert.Equal(
            "@(None->WithMetadataValue('Extension', '$(RunicToolkitMvvmBindingExtension)'))",
        ...
        Assert.Equal("@(RunicToolkitMvvmBinding)", RequiredAttribute(additionalFiles, "Include"));
    }

    private static void TargetsReconcileGeneratedSource()
    {
        XDocument targets = Load("buildTransitive", "RunicToolkit.MVVM.Build.targets");
        XElement compile = Target(targets, "RunicToolkitMvvmCompileBindings");
        ...
        XElement collect = Target(targets, "RunicToolkitMvvmCollectGeneratedBindings");
        ...
        XElement[] inventoryGuards = collect.Descendants("Error")
            .Where(static element => string.Equals((string?)element.Attribute("Code"), "RTKMVVM0903", StringComparison.Ordinal))
            .ToArray();
        ...
        Assert.Contains("@(_RunicToolkitMvvmGeneratedCompile", RequiredAttribute(generatedCompile, "Include"));
    ...
    {
        XDocument targets = Load("buildTransitive", "RunicToolkit.MVVM.Build.targets");
        string directory = Property(targets, "RunicToolkitMvvmBindingGeneratedDirectory");
        Assert.Equal("$(IntermediateOutputPath)RunicToolkit.MVVM.Bindings", directory,
            "Generated binding artifacts must remain under the project's intermediate output path.");

        XElement pathValidation = Target(targets, "RunicToolkitMvvmValidateBuildPaths");
        XElement containmentError = pathValidation.Descendants("Error").Single();
        Assert.Equal("RTKMVVM0902", RequiredAttribute(containmentError, "Code"));
        ...
        XElement clean = Target(targets, "RunicToolkitMvvmCleanRemovedBindings");
        Assert.Contains("@(RunicToolkitMvvmBinding)' == ''", RequiredAttribute(clean, "Condition"));
        Assert.Contains("RunicToolkitMvvmValidateBuildPaths", RequiredAttribute(clean, "DependsOnTargets"));
    ...
    private static XDocument Load(params string[] segments)
    {
        ...
        pathSegments[0] = "src";
        pathSegments[1] = "RunicToolkit.MVVM.Build";
