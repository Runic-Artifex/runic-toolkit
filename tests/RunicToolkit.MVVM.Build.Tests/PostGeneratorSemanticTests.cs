...
internal static class PostGeneratorSemanticTests
{
    ...
    private static int RunConsumer(GeneratedBindingArtifacts artifact)
    {
        string root = Path.Combine(
            Path.GetTempPath(),
            "RunicToolkit.MVVM.PostGeneratorConsumer",
    ...
    private static int RunNonNullableConsumer(GeneratedBindingArtifacts artifact)
    {
        string root = Path.Combine(
            Path.GetTempPath(),
            "RunicToolkit.MVVM.PostGeneratorNonNullableConsumer",
            Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
    ...
    private static int RunCollectionConsumer(GeneratedBindingArtifacts artifact)
    {
        string root = Path.Combine(
            Path.GetTempPath(),
            "RunicToolkit.MVVM.PostGeneratorCollectionConsumer",
            Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
            ...
            File.WriteAllText(Path.Combine(root, "Program.cs"), $$"""
                ...
                await using (var adapter =
                    new CommunityToolkitMvvmAdapterBuilder<GeneratedMemberViewModel>(viewModel)
                        ...
                        .Build())
                {
                    global::RunicToolkit.MVVM.MvvmSnapshot snapshot =
            ...
            File.WriteAllText(Path.Combine(root, "Consumer.csproj"), string.Create(
                ...
                <Project Sdk="Microsoft.NET.Sdk">
                  ...
                  <ItemGroup>
                    ...
                    <Reference Include="RunicToolkit.MVVM">
                      <HintPath>{SecurityElement.Escape(mvvmAssembly)}</HintPath>
                    </Reference>
                    <Reference Include="RunicToolkit.MVVM.CommunityToolkit">
    ...
    {
        string path = Path.Combine(AppContext.BaseDirectory, "RunicToolkit.MVVM.dll");
        Assert.True(File.Exists(path), "The RunicToolkit MVVM runtime is missing.");
        return path;
    }
    ...
    private static string CommunityToolkitAdapterAssemblyPath()
    {
        string path = Path.Combine(
            ...
            "RunicToolkit.MVVM.CommunityToolkit.dll");
