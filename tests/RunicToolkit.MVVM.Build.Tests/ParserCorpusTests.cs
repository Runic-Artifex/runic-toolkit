...
internal static class ParserCorpusTests
{
    ...
    private static void CompositeTypeSpellings()
    {
        ...
        {
            string source = $"protocol runic.toolkit.mvvm/1; contract \"case{index}\" model Example.Model {{ property 1 value: {types[index]} => Value readonly; }}";
            BindingSemanticResult result = BindingCompiler.Compile(source, $"corpus/type-{index}.rtkmvvm");
    ...
    private static void InvalidContracts()
    {
        ...
        [
            ...
            ("protocol runic.toolkit.mvvm/2; contract \"x\" model X {}", BindingDiagnosticIds.ProtocolMismatch),
            ("protocol runic.toolkit.mvvm/1; contract \"x\" model X { property 0 value: X => Value readonly; }", BindingDiagnosticIds.InvalidMemberId),
            ("protocol runic.toolkit.mvvm/1; contract \"x\" model X { property 1 value: none => Value readonly; }", BindingDiagnosticIds.InvalidTypeName),
        ];

        for (int index = 0; index < cases.Length; index++)
        {
            BindingSemanticResult result = BindingCompiler.Compile(cases[index].Source, $"corpus/invalid-{index}.rtkmvvm");
