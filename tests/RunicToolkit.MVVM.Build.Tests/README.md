# RunicToolkit.MVVM.Build executable contracts
...
```console
dotnet restore tests/RunicToolkit.MVVM.Build.Tests/RunicToolkit.MVVM.Build.Tests.csproj -p:RunicToolkitLocalPackageSource=<packed-feed>
dotnet run --project tests/RunicToolkit.MVVM.Build.Tests/RunicToolkit.MVVM.Build.Tests.csproj --configuration Release --no-restore
```

The packed feed must contain `RunicToolkit.MVVM` version `1.0.0`; repository
...
`RunicToolkit.MVVM.Build.Symbols.GeneratedMemberContractCompiler`, package
`CommunityToolkit.Mvvm` `8.4.2`, and the reserved diagnostics
`RTKMVVM2014` through `RTKMVVM2018`. Its Flow handoff is deliberately declarative:
