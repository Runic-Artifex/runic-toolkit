...
internal sealed record TestScenario(string Id, Func<ValueTask> ExecuteAsync);

internal static class TestRunner
{
    public static async Task<int> RunAsync(IReadOnlyList<TestScenario> scenarios)
    {
        ...
        {
            TestScenario scenario = scenarios[index];
            try
            {
...
internal static class TestAssert
{
    ...
    public static void EqualSequence<T>(IReadOnlyList<T> expected, IReadOnlyList<T> actual, string? because = null)
    {
        ...
        for (var index = 0; index < expected.Count; index++)
        {
            ...
            {
                Fail($"Sequences differ at {index}: expected <{expected[index]}> but found <{actual[index]}>.{Reason(because)}");
            }
        }
    }
    ...
    private static void Fail(string message) => throw new TestAssertionException(message);
    private static bool IsProcessFatal(Exception exception) =>
        exception is OutOfMemoryException or StackOverflowException or AccessViolationException;
}
