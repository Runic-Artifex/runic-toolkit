...
$libraryProject = Join-Path $repositoryDirectory "src/RunicToolkit.MVVM/RunicToolkit.MVVM.csproj"
$smokeProject = Join-Path $smokeDirectory "RunicToolkit.MVVM.AotSmoke.csproj"
$temporaryDirectory = Join-Path ([System.IO.Path]::GetTempPath()) ("runic-toolkit-mvvm-aot-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Packing RunicToolkit.MVVM failed." }
    ...
    $windowsExecutable = Join-Path $publishDirectory "RunicToolkit.MVVM.AotSmoke.exe"
    $executablePath = if (Test-Path -LiteralPath $windowsExecutable) {
        $windowsExecutable
    }
    else {
        Join-Path $publishDirectory "RunicToolkit.MVVM.AotSmoke"
    }
    & $executablePath
