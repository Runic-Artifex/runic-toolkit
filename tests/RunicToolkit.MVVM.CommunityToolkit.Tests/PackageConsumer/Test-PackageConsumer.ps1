...
"@ | Set-Content -Encoding UTF8 $config
    ...
    Invoke-DotNet @("pack", "-c", $Configuration, "--no-restore", "-p:PackageVersion=0.0.0-local",
        "-p:ContinuousIntegrationBuild=true", "-p:RunicToolkitBuildMode=Verification",
        "-o", $feed, (Join-Path $root "src/RunicToolkit.MVVM/RunicToolkit.MVVM.csproj"))
    Invoke-DotNet @("pack", "-c", $Configuration, "--no-restore", "-p:PackageVersion=0.0.0-local",
        "-p:ContinuousIntegrationBuild=true", "-p:RunicToolkitBuildMode=Verification",
        "-o", $feed, (Join-Path $root "src/RunicToolkit.MVVM.CommunityToolkit/RunicToolkit.MVVM.CommunityToolkit.csproj"))

    $runtimePackage = Join-Path $feed "RunicToolkit.MVVM.0.0.0-local.nupkg"
    $adapterPackage = Join-Path $feed "RunicToolkit.MVVM.CommunityToolkit.0.0.0-local.nupkg"
    Assert-PackageDependencies -Path $runtimePackage -Expected @{}
    Assert-PackageDependencies -Path $adapterPackage -Expected @{
        "CommunityToolkit.Mvvm" = "[8.4.2]"
        "RunicToolkit.MVVM" = "0.0.0-local"
    }
    ...
    if (-not $SkipNativeAot) {
        ...
        $nativeExecutableName = if ($RuntimeIdentifier.StartsWith('win-', [StringComparison]::OrdinalIgnoreCase)) {
            'RunicToolkit.MVVM.CommunityToolkit.PackageConsumer.exe'
        } else {
            'RunicToolkit.MVVM.CommunityToolkit.PackageConsumer'
        }
