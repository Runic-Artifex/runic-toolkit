...
```powershell
./tests/RunicToolkit.Collections.AotSmoke/run-native-smoke.ps1
```

Release jobs may state their matching RID explicitly:

```powershell
./tests/RunicToolkit.Collections.AotSmoke/run-native-smoke.ps1 -RuntimeIdentifier win-x64
./tests/RunicToolkit.Collections.AotSmoke/run-native-smoke.ps1 -RuntimeIdentifier linux-x64
./tests/RunicToolkit.Collections.AotSmoke/run-native-smoke.ps1 -RuntimeIdentifier osx-arm64
```
...
```console
dotnet build -c Release src/RunicToolkit.Collections
dotnet restore tests/RunicToolkit.Collections.AotSmoke
dotnet run -c Release --no-restore --project tests/RunicToolkit.Collections.AotSmoke
dotnet restore -r <rid> --disable-parallel -p:PublishAot=true -p:PublishTrimmed=true tests/RunicToolkit.Collections.AotSmoke
dotnet publish -c Release -r <rid> --no-restore -p:PublishAot=true -p:PublishTrimmed=true tests/RunicToolkit.Collections.AotSmoke
```
