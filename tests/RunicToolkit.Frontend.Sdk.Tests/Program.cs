...
namespace RunicToolkit.Frontend.Sdk.Tests;
...
internal static class Program
{
    ...
    private static readonly string SdkProps = Path.Combine(
        ...
        "RunicToolkit.Frontend.Sdk",
        "buildTransitive",
        "RunicToolkit.Frontend.Sdk.props");
    private static readonly string SdkTargets = Path.Combine(
        RepositoryRoot,
        "src",
        "RunicToolkit.Frontend.Sdk",
        "buildTransitive",
        "RunicToolkit.Frontend.Sdk.targets");
    ...
    public static int Main(string[] args)
    {
        ...
        [
            ...
            ("SDK validation accepts a Node-free compiler pipeline", SdkValidationAcceptsCompilerOnlyConfiguration),
    ...
    private static void SdkValidationAcceptsCompleteConfiguration()
    {
        ...
        string project = workspace.WriteText(
            ...
            CreateValidationProject(
                """
                <RunicToolkitFrontendWorkspace>fixture</RunicToolkitFrontendWorkspace>
                <RunicToolkitFrontendPackageDirectory>frontend</RunicToolkitFrontendPackageDirectory>
                """));

        ProcessResult result = Run(
            ResolveDotNetHost(),
            workspace.Root,
            ["msbuild", project, "-nologo", "-verbosity:minimal", "-target:RunicToolkitFrontendValidate"]);
    ...
    private static void SdkValidationRejectsIncompleteContractOutputs()
    {
        ...
        string project = workspace.WriteText(
            ...
            CreateValidationProject(
                ...
                <RunicToolkitFrontendWorkspace>fixture</RunicToolkitFrontendWorkspace>
                <RunicToolkitFrontendPackageDirectory>frontend</RunicToolkitFrontendPackageDirectory>
                <RunicToolkitFrontendContractSource>contract.json</RunicToolkitFrontendContractSource>
                <RunicToolkitFrontendContractCSharpOutput>Contract.g.cs</RunicToolkitFrontendContractCSharpOutput>
                """));
    ...
    private static void SdkValidationAcceptsCompilerOnlyConfiguration()
    {
        using TestWorkspace workspace = new();
        string project = workspace.WriteText(
            "compiler-only.proj",
            CreateValidationProject(
                """
                <RunicToolkitFrontendNodeEnabled>false</RunicToolkitFrontendNodeEnabled>
                <RunicToolkitFrontendCompilerEnabled>true</RunicToolkitFrontendCompilerEnabled>
                """));
        ...
        ProcessResult result = Run(
            ...
            workspace.Root,
            ["msbuild", project, "-nologo", "-verbosity:minimal", "-target:RunicToolkitFrontendValidate"]);
        AssertSuccess(result, "compiler-only SDK target validation");
    }
    ...
    private static void SdkValidationRejectsMissingPipeline()
    {
        ...
        string project = workspace.WriteText(
            ...
            CreateValidationProject(
                """
                <RunicToolkitFrontendNodeEnabled>false</RunicToolkitFrontendNodeEnabled>
                <RunicToolkitFrontendCompilerEnabled>false</RunicToolkitFrontendCompilerEnabled>
                """));
        ...
        AssertContains(
            result.CombinedOutput,
            "Enable at least one Runic Toolkit frontend pipeline",
    ...
    private static void SdkInstallCacheFollowsLockFileIdentity()
    {
        ...
        string project = workspace.WriteText(
            ...
            CreateValidationProject(
                ...
                <RunicToolkitFrontendWorkspace>fixture</RunicToolkitFrontendWorkspace>
                <RunicToolkitFrontendPackageDirectory>frontend</RunicToolkitFrontendPackageDirectory>
                <RunicToolkitFrontendInstallCommand>{SecurityElement.Escape(installCommand)}</RunicToolkitFrontendInstallCommand>
                """));

        AssertSuccess(RunSdkTarget(workspace.Root, project, "RunicToolkitFrontendInstall"), "Initial SDK frontend install");
        AssertLineCount(counter, 1, "The initial frontend install did not execute exactly once.");

        AssertSuccess(RunSdkTarget(workspace.Root, project, "RunicToolkitFrontendInstall"), "Cached SDK frontend install");
        AssertLineCount(counter, 1, "An unchanged lock file caused a redundant frontend install.");

        File.AppendAllText(lockFile, "\n", new UTF8Encoding(false));
        AssertSuccess(RunSdkTarget(workspace.Root, project, "RunicToolkitFrontendInstall"), "Invalidated SDK frontend install");
        ...
        AssertSuccess(
            RunSdkTarget(
                workspace.Root,
                project,
                "RunicToolkitFrontendInstall",
                "-property:RunicToolkitFrontendPackageManager=custom",
                $"-property:RunicToolkitFrontendLockFile={lockFile}"),
    ...
    private static void SdkInstallIsInertWhenDisabled()
    {
        ...
        string noRestoreProject = workspace.WriteText(
            ...
            CreateValidationProject(
                ...
                <RunicToolkitFrontendWorkspace>fixture</RunicToolkitFrontendWorkspace>
                <RunicToolkitFrontendPackageDirectory>frontend</RunicToolkitFrontendPackageDirectory>
                <RunicToolkitFrontendInstall>false</RunicToolkitFrontendInstall>
                """));
        AssertSuccess(
            RunSdkTarget(workspace.Root, noRestoreProject, "RunicToolkitFrontendInstall"),
        ...
        string nodeFreeProject = workspace.WriteText(
            ...
            CreateValidationProject(
                ...
                """));
        AssertSuccess(
            RunSdkTarget(workspace.Root, nodeFreeProject, "RunicToolkitFrontendInstall"),
    ...
    private static void SdkSerializesConcurrentInstalls()
    {
        ...
        Task<ProcessResult> firstInstall = Task.Run(
            () => RunSdkTarget(workspace.Root, first, "RunicToolkitFrontendInstall"));
        Task<ProcessResult> secondInstall = Task.Run(
            () => RunSdkTarget(workspace.Root, second, "RunicToolkitFrontendInstall"));
    ...
    private static void SdkBuildOptOutSkipsProductionAssets()
    {
        ...
        string project = workspace.WriteText(
            ...
            CreateValidationProject(
                ...
                <RunicToolkitFrontendPackageDirectory>frontend</RunicToolkitFrontendPackageDirectory>
                <RunicToolkitFrontendInstall>false</RunicToolkitFrontendInstall>
                <RunicToolkitFrontendBuild>false</RunicToolkitFrontendBuild>
                <RunicToolkitFrontendBuildCommand>{SecurityElement.Escape(buildCommand)}</RunicToolkitFrontendBuildCommand>
                <RunicToolkitFrontendViteDevServerEnabled>true</RunicToolkitFrontendViteDevServerEnabled>
                <TargetDir>{SecurityElement.Escape(targetDirectory)}</TargetDir>
                """));

        AssertSuccess(
            RunSdkTarget(workspace.Root, project, "RunicToolkitFrontendCopyToOutput"),
            "Vite development-assets SDK build");

        string runtimeWebRoot = Path.Combine(targetDirectory, "www");
        AssertFalse(
            File.Exists(buildCounter),
            "The production frontend build executed even though RunicToolkitFrontendBuild was false.");
        ...
        AssertSuccess(
            RunSdkTarget(
                workspace.Root,
                project,
                "RunicToolkitFrontendCopyToOutput",
                "-property:RunicToolkitFrontendBuild=true"),
        ...
        {
            throw new InvalidOperationException(
                "The production frontend asset was not copied when RunicToolkitFrontendBuild was true.");
        }
    }
    ...
    private static string CreateValidationProject(string properties)
    {
        ...
        return $"""
            <Project>
              ...
              <PropertyGroup>
                <RunicToolkitFrontendEnabled>true</RunicToolkitFrontendEnabled>
                {properties}
              </PropertyGroup>
    ...
    private static string FindRepositoryRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "RunicToolkit.slnx")))
            {
        ...
        throw new InvalidOperationException(
            $"Could not find RunicToolkit.slnx from '{AppContext.BaseDirectory}'.");
    }
