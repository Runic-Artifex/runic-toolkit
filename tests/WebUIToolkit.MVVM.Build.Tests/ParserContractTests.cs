...
internal static class ParserContractTests
{
    ...
    {
        BindingParseResult result = BindingParser.Parse(ValidSource, "bindings/settings.wutmvvm");

        Assert.False(result.HasErrors, DiagnosticsText(result.Diagnostics));
        Assert.Equal("webuitoolkit.mvvm/1", result.Syntax.Protocol?.Identity);
    ...
    {
        const string source = "protocol webuitoolkit.mvvm/1;\r\ncontract \"settings\" model Example.Model {\r\n@\r\n}\r\n";
        const string path = "diagnostics/exact-span.wutmvvm";
    ...
    private static void KeepsLogicalPaths()
    {
        ...
        BindingSemanticResult first = BindingCompiler.Compile(malformed, "one/contract.wutmvvm");
        BindingSemanticResult second = BindingCompiler.Compile(malformed, "other/contract.wutmvvm");
        ...
        Assert.Equal("one/contract.wutmvvm", firstDiagnostic.Span.LogicalPath);
        Assert.Equal("other/contract.wutmvvm", secondDiagnostic.Span.LogicalPath);
    }
