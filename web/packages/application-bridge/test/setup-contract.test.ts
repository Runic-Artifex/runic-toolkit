...
import setup from "../../../../protocol/application-bridge/setup/contract.mjs";
...
test("committed Setup fixtures decode through the Effect wire schemas", async () => {
  ...
  const command = setup.commands.find((item) => item.tag === "InitializeApplication");
  assert.ok(command);
  assert.deepEqual(
  ...
  assert.deepEqual(
    await Effect.runPromise(Schema.decodeUnknown(setup.schemas.SetupSnapshot)(decodedHost.payload)),
    host.payload,
  );
});
