...
try {
    ...
    Write-Host 'Packing the repository-local Runic Application development command.'
    ...
    Write-Host 'Packing the repository-local Runic Application templates.'
    Invoke-DotNet -Arguments @(
        'pack',
        $templateProject,
        '--configuration', $Configuration,
    ...
    Write-Host ''
    Write-Host 'Runic Application development setup is ready.'
    Write-Host 'Run: dotnet runic doctor <PROJECT>'
    Write-Host 'Then: dotnet runic dev <PROJECT>'
}
finally {
    Pop-Location
}
