...
internal static class InspectApplication
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (evaluation.ExitCode != 0)
        {
            throw new DevUsageException(
                "WUTDEV1010",
                $"Could not evaluate C# markup artifacts for '{project}'.");
        }
        ...
        bool hasCsharpMarkup = response.RootElement.TryGetProperty("Items", out JsonElement items) &&
        ...
        if (!active && !hasCsharpMarkup)
        {
            throw new DevUsageException(
                "WUTDEV1010",
                "The selected project has no active .cwuix documents.");
        }

        if (artifact == "generated")
        {
            string configuredGeneratedRoot = Value("CompilerGeneratedFilesOutputPath");
            if (string.IsNullOrWhiteSpace(configuredGeneratedRoot))
            {
                configuredGeneratedRoot = Path.Combine(
        ...
        {
            "manifest" => "WebUIToolkitCsharpMarkupManifestPath",
            "diagnostics" => "WebUIToolkitCsharpMarkupDiagnosticsPath",
            "hot-reload" => "WebUIToolkitCsharpMarkupHotReloadPath",
            _ => throw new InvalidOperationException(),
        };
        string path = Path.GetFullPath(Value(property), directory);
        if (!File.Exists(path))
        {
        ...
        string[] arguments)
    {
        ...
        for (int index = 1; index < arguments.Length; index++)
        {
            ...
            if (argument is "--configuration" or "--artifact")
            {
                ...
                {
                    throw new DevUsageException("WUTDEV1001", $"{argument} requires a value.");
                }
            ...
            {
                throw new DevUsageException("WUTDEV1001", $"Unknown option '{argument}'.");
            }
            ...
            {
                throw new DevUsageException("WUTDEV1001", "Specify at most one project.");
            }
        }
    ...
    private static DevDevelopmentException MissingArtifact(string name, string path) =>
        new(
            "WUTDEV1011",
            $"The C# markup {name} artifact '{path}' does not exist. Build the project first.");

    private static void WriteHelp() => Console.WriteLine(
        """
        Usage:
          dotnet webuitoolkit inspect [PROJECT] [options]
